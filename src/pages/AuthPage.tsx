...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-white dark:bg-slate-950 transition-colors duration-500">
      ...
      <motion.div 
        ...
        className="w-full max-w-md bg-white dark:bg-white/[0.03] backdrop-blur-3xl border border-black/10 dark:border-white/10 p-10 rounded-[2rem] shadow-2xl transition-colors duration-500"
      >
        ...
        {error && <div className="p-3 mb-6 bg-red-500/10 border border-red-500/30 text-red-500 dark:text-red-400 text-[10px] uppercase font-bold tracking-widest rounded-lg text-center font-mono">{error}</div>}
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-slate-50 dark:bg-slate-950 border border-black/5 dark:border-white/5 rounded-xl py-4 pl-12 pr-4 focus:ring-1 focus:ring-cyan-500 outline-none transition-all text-sm text-slate-900 dark:text-white font-medium"
              value={email}
        ...
        <div className="relative my-10">
          ...
          <div className="relative flex justify-center text-[10px] uppercase font-black"><span className="bg-white dark:bg-[#020617] px-4 text-slate-400 dark:text-slate-600 tracking-widest transition-colors font-mono">Auth_Gateway</span></div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <button onClick={() => handleProviderAuth(googleProvider)} className="py-3 border border-black/5 dark:border-white/10 bg-black/5 dark:bg-white/5 rounded-xl flex items-center justify-center gap-3 hover:bg-black/10 dark:hover:bg-white/10 transition-all text-xs font-bold text-slate-600 dark:text-slate-300">
          ...
          <button onClick={() => handleProviderAuth(githubProvider)} className="py-3 border border-black/5 dark:border-white/10 bg-black/5 dark:bg-white/5 rounded-xl flex items-center justify-center gap-3 hover:bg-black/10 dark:hover:bg-white/10 transition-all text-xs font-bold text-slate-600 dark:text-slate-300">
