            ...
            <form className="relative z-10 space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-3 px-1">
                  ...
                  <input type="text" className="w-full bg-white dark:bg-black border border-black/10 dark:border-white/10 rounded-2xl p-5 text-sm font-bold uppercase text-slate-950 dark:text-white focus:border-cyan-500 outline-none transition-all shadow-sm font-mono" placeholder="John Matrix" />
                </div>
                <div className="space-y-3 px-1">
                  <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Auth Email</label>
                  <input type="email" className="w-full bg-white dark:bg-black border border-black/10 dark:border-white/10 rounded-2xl p-5 text-sm font-bold uppercase text-slate-950 dark:text-white focus:border-cyan-500 outline-none transition-all shadow-sm font-mono" placeholder="operator@neural.net" />
                </div>
              </div>
              <div className="space-y-3 px-1">
                <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Subject Protocol</label>
                <input type="text" className="w-full bg-white dark:bg-black border border-black/10 dark:border-white/10 rounded-2xl p-5 text-sm font-bold uppercase text-slate-950 dark:text-white focus:border-cyan-500 outline-none transition-all shadow-sm font-mono" placeholder="Neural Tier Upgrade Inquiry" />
              </div>
              <div className="space-y-3 px-1">
                <label className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest">Message Payload</label>
                <textarea rows={5} className="w-full bg-white dark:bg-black border border-black/10 dark:border-white/10 rounded-2xl p-5 text-sm font-bold uppercase text-slate-950 dark:text-white focus:border-cyan-500 outline-none transition-all resize-none shadow-sm font-mono" placeholder="Describe your neural synchronization requirement..." />
              </div>
              <button className="w-full py-5 bg-slate-950 dark:bg-cyan-500 hover:bg-slate-900 dark:hover:bg-cyan-400 text-white dark:text-slate-950 text-xs font-bold rounded-2xl uppercase tracking-widest transition-all shadow-xl shadow-cyan-500/10 active:scale-95">
                Transmit Signal
              </button>
            </form>
          </div>
